...
- **Broadcast notifications** — the estate-wide operational nudges (a critical-app grant, an admin
  ...
  offline agent, an expiring asset warranty or access grant) are visible only to people whose role
  holds the **notification** permission. By default that is **administrators only**.
...
| **Duplicate machine ID** | Two servers reported the same machine ID but different hardware (a different serial *and* different network cards) — almost always a VM template or golden image cloned with one baked in. Both are listed separately (nothing is merged) and the notification names the fix. One notification per colliding host, not one per check-in. Links to the topology map. |
...
| **New AI agent connected** | (Targeted, security) An external AI agent — Claude Code, Cursor or another MCP client, or a personal MCP token — was used on your account for the first time. One per connection. Lands in your bell — and inbox, unless you opt out. If it wasn't you, revoke the connection in your AI connections and change your password. |
