...
- Workflow configuration and run history are visible only to people who hold the automation permissions (by default, the ADMIN role only). Some applications are marked critical: changing their access or automation is possible only in the lazyit chat, where it needs extra confirmation.
