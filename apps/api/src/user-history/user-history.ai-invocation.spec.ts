 ...
 */
describe('UserHistoryService — aiInvocationId stamp', () => {
  const EVENT = {
  ...
  function writer() {
    return { userHistory: { create: jest.fn().mockResolvedValue({}) } };
  }

  function writtenData(client: ReturnType<typeof writer>) {
    const calls = client.userHistory.create.mock.calls as Array<
      [{ data: Record<string, unknown> }]
    >;
    return calls[0][0].data;
  }

  const service = new UserHistoryService({} as never);
