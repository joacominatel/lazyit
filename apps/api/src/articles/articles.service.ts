...
export class ArticlesService {
  ...
  /**
   * ADR-0082: may the principal attach/detach FILES on this article? Exactly the edit gate — the
   * author, or an ADMIN / `article:manage` holder (who must still pass the folder ACL): 404 for a
   * missing / soft-deleted / folder-hidden article or a foreign DRAFT (existence-hiding), 403 for a
   * published article the caller may not edit, 403 for a service account (an uploader is a human).
   * Public so the AttachmentsModule can enforce the parent's write rule without duplicating it.
   */
  async assertAttachmentWritable(
    id: string,
    principal?: Principal,
  ): Promise<void> {
