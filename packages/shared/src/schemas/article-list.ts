...
/**
 * `?linkedTo=` narrows the linked filter to a single target kind (ADR-0042): `asset` keeps only
 * articles linked to ≥1 Asset, `application` only those linked to ≥1 Application. Same allowlist /
 * 400-on-unknown contract as {@link ArticleLinkedFilterSchema}. Meaningful only alongside
 * `linked=only`; on its own it is treated as an implicit `linked=only` narrowed to that target.
 */
