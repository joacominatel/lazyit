...
- **Designing the AI assistant?** The opt-in [[ai-assistant/_MOC|AI Assistant, MCP server and headless
  API]] design vault (proposed, not built) — read its synthesis first; decision record
...
| `ai-assistant` | [[ai-assistant/_MOC\|AI Assistant]] | Design vault (proposed) for the AI chat, MCP server with lazyit's own OAuth 2.1, and headless API — providers, runtime, tool catalog, frontend, security |
...
- **Knowledge Base v2 + Secret Manager — BUILT.** Three ADRs ratified 2026-06-11; all three are now
  ...
  - **KB v2 structure** — folders (the flat [[article-category]] evolved into a hierarchy + one home
    ...
    and bulk `.zip` import ([[0059-kb-folders-links-and-import]], #364). **Built — merged on
    `feat/kb-secrets`.**
  - **Folder-based KB access** — the **folder** as the permission boundary (per-folder carve-out from
    the per-domain-only rule of [[0040-rbac-roles]]/[[0046-roles-permissions-v2]]); default PUBLIC,
    additive dynamic rules, API+DB enforcement, no-escalation; **INV-9** enforced
    ([[0060-kb-folder-access-control]], #365). **Built — merged on `feat/kb-secrets`.**
  - **Zero-knowledge Secret Manager** — vaults beside the KB where the server can **never** decrypt a
    secret value (per-user X25519 keypair, Argon2id passphrase-lock, DEK wrapped per member via
    ECIES/HKDF/AES-GCM, recovery key shown once, peer-reset); a **third** secret store distinct from
    the server-decryptable [[workflow-secret]]; `secret:read`/`secret:manage` capability; **INV-10**
    (the INV-8 crypto-exception) enforced by the INV-10 architectural guard test. KB chip
    `{{ lazyit_secret.HANDLE }}` with double gate. ([[0061-secret-manager-zero-knowledge]], #366).
    **Built — merged on `feat/secret-manager`.**
