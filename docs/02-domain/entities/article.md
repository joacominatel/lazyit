...
- **references / referenced by** other [[article]]s via `[[slug]]` wiki-links — 🟢 **implemented**
  (#392, [[0059-kb-folders-links-and-import]] §3/§4): a materialized [[article-wiki-link]] edge is
  **hard-rebuilt on every content-changing save, in the write transaction** (source → `targetSlug`,
...
- **Slug** is unique, short and memorable (`[a-z0-9-]`, ≤60 chars). Autogenerated from the title
  (`slugify` in `@lazyit/shared`) but **editable**. The **slug-auto-suffix** util (`nextAvailableSlug`
  in `@lazyit/shared`, `-2`/`-3`/… on collision) shipped in #392 — resolving the
  [[0021-knowledge-base-design]] open question in code ([[0059-kb-folders-links-and-import]] §3) — and
  is consumed by the (later) bulk import to mint dozens of slugs frictionlessly; the **interactive
  single create still returns `409` on collision today** (wiring the suffix into that path is a small
  follow-up).
