...
| **Asymmetric (keypair + ECDH)** | **`@noble/curves@2.4.0`** (`x25519`) | Audited, minimal X25519. `x25519.keygen()` / `x25519.getSharedSecret()`. 32-byte keys → tiny wrapped blobs. | **Browser only** (and tests). |
| **AEAD — AES-256-GCM** | **`@noble/ciphers@2.4.0`** (`gcm`) | Audited AES-GCM that produces the **same envelope shape** as the `node:crypto` `WorkflowSecret` path (ciphertext ‖ 16-byte tag, explicit 12-byte nonce), so client and server agree byte-for-byte on the `SecretItem` columns. | **Browser only** (and tests). |
| **HKDF (shared-secret → AES key)** | **`@noble/hashes@2.4.0`** (`hkdf` + `sha256`) | Audited HKDF-SHA-256 to expand the raw X25519 shared secret into a clean 32-byte AES key (never use the raw DH output directly as a key). | **Browser only** (and tests). |
...
`@noble/curves@2.4.0` is audited and runs everywhere we need.
